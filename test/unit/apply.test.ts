...
describe('apply commands', () => {
  ...
  it('runs an apply command to create a module', () => {
    const datacenterid = 'datacenter-id';
    const inputs: [string, string][] = [['input1', '1']];
    const image = 'image-digest';
    pulumi_module.apply({ datacenterid, inputs, image }, mock_websocket_connection as unknown as WebSocket);

    const docker_command_args = spawn_stub.firstCall.args;
    const docker_run_args = docker_command_args[1];
    const pulumi_command = docker_run_args[7].split('\n').map((c: string) => c.trim());
    
    expect(docker_command_args[0]).to.equal('docker');
    expect(docker_run_args[0]).to.equal('run');
    expect(docker_run_args[1]).to.equal('--entrypoint');
    expect(docker_run_args[2]).to.equal('bash');
    expect(docker_run_args[3]).to.equal('-e');
    expect(docker_run_args[4]).to.equal('PULUMI_CONFIG_PASSPHRASE=');
    expect(docker_run_args[5]).to.equal(image);
    expect(docker_run_args[6]).to.equal('-c');
    ...
    expect(pulumi_command[7]).to.equal('echo ****PULUMI_DELIMITER**** &&');
    expect(pulumi_command[8]).to.equal(`pulumi stack export --stack ${datacenterid} &&`);
    expect(pulumi_command[9]).to.equal('echo ****PULUMI_DELIMITER**** &&');
  ...
  it('runs an apply command to delete a module', () => {
    const datacenterid = 'datacenter-id';
    const inputs: [string, string][] = [['input1', '1']];
    const image = 'image-digest';
    const destroy = true;
    const state = '{}';
    pulumi_module.apply({ datacenterid, inputs, image, destroy, state }, mock_websocket_connection as unknown as WebSocket);
    ...
    expect(docker_run_args[5]).to.equal(image);
    expect(docker_run_args[6]).to.equal('-c');
    expect(pulumi_command[0]).to.equal(`echo ${JSON.stringify(state).replace(/"/g, '\'')} > pulumi-state.json`);
    expect(pulumi_command[1]).to.equal('pulumi login --local &&');
    expect(pulumi_command[2]).to.equal(`pulumi stack init --stack ${datacenterid} &&`);
